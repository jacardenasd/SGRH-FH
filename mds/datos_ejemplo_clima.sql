...
-- Los valores estarán distribuidos para un promedio de 66% (escala 1-3 = 1.66)
...
SELECT 
  ...
  crt.reactivo_id,
  CASE 
    WHEN RAND() < 0.30 THEN 1  -- 30% respuestas altas (De acuerdo)
    WHEN RAND() < 0.40 THEN 2  -- 40% respuestas medias (Parcialmente de acuerdo)
    ELSE 3                      -- 30% respuestas bajas (En desacuerdo)
  END AS valor,
...
SELECT 
  ...
  crt.reactivo_id,
  CASE 
    WHEN RAND() < 0.20 THEN 1  -- 20% respuestas altas (De acuerdo)
    WHEN RAND() < 0.35 THEN 2  -- 35% respuestas medias (Parcialmente de acuerdo)
    ELSE 3                      -- 45% respuestas bajas (En desacuerdo)
  END AS valor,
  NOW() AS fecha_respuesta
FROM clima_elegibles ce
CROSS JOIN (
...
SELECT 'VERIFICACIÓN - Promedio general (debe ser ~1.66 = 66%)' AS verificacion;
SELECT AVG(valor) AS promedio_1_3, 
       ROUND((3 - AVG(valor)) / 2 * 100, 2) AS promedio_0_100
FROM clima_respuestas WHERE periodo_id = 1;
...
SELECT 'VERIFICACIÓN - Promedio por unidad 1 (debe ser ~1.35 = 45%)' AS verificacion;
SELECT AVG(cr.valor) AS promedio_1_3,
       ROUND((3 - AVG(cr.valor)) / 2 * 100, 2) AS promedio_0_100
FROM clima_respuestas cr
INNER JOIN clima_elegibles ce ON cr.periodo_id = ce.periodo_id AND cr.empleado_id = ce.empleado_id
WHERE cr.periodo_id = 1 AND ce.unidad_id = 1;
...
SELECT 
  ...
  ROUND(AVG(cr.valor), 2) AS promedio_1_3,
  ROUND((3 - AVG(cr.valor)) / 2 * 100, 2) AS promedio_0_100
FROM clima_respuestas cr

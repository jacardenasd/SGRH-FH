...
CREATE TABLE `clima_respuestas`  (
  ...
  UNIQUE INDEX `idx_periodo_empleado_reactivo`(`periodo_id`, `empleado_id`, `reactivo_id`, `unidad_id`) USING BTREE,
  ...
  INDEX `idx_clima_resp_unidad`(`periodo_id`, `unidad_id`) USING BTREE,
  ...
  INDEX `idx_periodo_unidad_reactivo`(`periodo_id`, `unidad_id`, `reactivo_id`) USING BTREE,
  UNIQUE INDEX `idx_periodo_empleado_reactivo_unico`(`periodo_id`, `empleado_id`, `reactivo_id`) USING BTREE,
...
) ENGINE = InnoDB AUTO_INCREMENT = 11186 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Dynamic;
...
CREATE TABLE `empleados`  (
  ...
  `sueldo_integrado` decimal(12, 2) NULL DEFAULT NULL COMMENT 'Sueldo integrado (diario + prestaciones)',
...
) ENGINE = InnoDB AUTO_INCREMENT = 14586 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Dynamic;
...
CREATE TABLE `nomina_importaciones`  (
  ...
  `status` enum('cargado','procesado','error','revertido') CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL DEFAULT 'cargado',
...
CREATE TABLE `org_plantilla_autorizada`  (
  ...
  `fecha_cancelacion` date NULL DEFAULT NULL COMMENT 'Fecha de cancelacion',
  `fecha_congelacion` date NULL DEFAULT NULL COMMENT 'Fecha de congelacion',
  ...
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `sindicalizado` enum('si','no') CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT 'no' COMMENT 'Indica si la plaza es sindicalizada',
  `tipo_plaza` enum('planta','temporal') CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT 'planta' COMMENT 'Tipo de plaza: planta o temporal',
  `nivel` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT 'Nivel jer??rquico',
  ...
  INDEX `idx_plaza_sindicalizado`(`sindicalizado`) USING BTREE,
  INDEX `idx_plaza_tipo_plaza`(`tipo_plaza`) USING BTREE,
  INDEX `idx_plaza_nivel`(`nivel`) USING BTREE,
...
) ENGINE = InnoDB AUTO_INCREMENT = 1106 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = 'Registro individual de plazas autorizadas' ROW_FORMAT = DYNAMIC;
...
CREATE TABLE `usuarios_accesos`  (
  ...
  INDEX `fk_accesos_empresa`(`empresa_id`) USING BTREE,
...
) ENGINE = InnoDB AUTO_INCREMENT = 9 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = 'Registro de accesos (login) de usuarios al sistema' ROW_FORMAT = DYNAMIC;
...
-- ----------------------------
-- View structure for moodle_auth_users
-- ----------------------------
DROP VIEW IF EXISTS `moodle_auth_users`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `moodle_auth_users` AS select concat(`usuarios`.`no_emp`,'_',`usuarios`.`rfc_base`) AS `username`,`usuarios`.`password_hash` AS `password`,`usuarios`.`nombre` AS `firstname`,trim(concat(ifnull(`usuarios`.`apellido_paterno`,''),' ',ifnull(`usuarios`.`apellido_materno`,''))) AS `lastname`,`usuarios`.`correo` AS `email` from `usuarios` where ((`usuarios`.`estatus` = 'activo') and (`usuarios`.`correo` is not null) and (`usuarios`.`correo` <> ''));
...
-- ----------------------------
-- Function structure for curp_extraer_fecha_nacimiento
-- ----------------------------
...
-- ----------------------------
-- Function structure for curp_extraer_lugar_nacimiento
-- ----------------------------
...
-- ----------------------------
-- Function structure for curp_extraer_nacionalidad
-- ----------------------------
